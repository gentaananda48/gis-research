...
class GenerateRencanaKerjaReport extends Command
{
    ...
    public function handle()
    {
        ...
        foreach($list_rk AS $rk) {
            //find report rencana kerja by condition and delete to get a new data
            ReportRencanaKerja::where('rencana_kerja_id', $rk->id)->delete();
            $list_polygon = $geofenceHelper->createListPolygon('L', $rk->lokasi_kode);
            //create polygon for calculating ritas that will be use for summary and data percentage
            ...
            //this is for looping area to find calculation ritase and is that overlapping ot not
            $ritase = 0;
