...
class LacakController extends Controller {
    ...
    public function create3(Request $request){
		DB::beginTransaction();
	    try {
	    	$source_device_id = !empty($request->source_device_id) ? $request->source_device_id : '';
	    	$table_name = 'lacak_'.$source_device_id;
	    	if (Schema::hasTable($table_name)) {
	    		$speed = !empty($request->speed) ? round($request->speed,2) : 0;
	    		$report_date = date('His', $request->utc_timestamp) <= '050000' ? date('Y-m-d', strtotime("-1 day", $request->utc_timestamp)) : date('Y-m-d', $request->utc_timestamp);
	    		DB::insert('insert into lacak_'.$source_device_id.' (latitude, longitude, speed, altitude, arm_height_left, arm_height_right, temperature_left, temperature_right, pump_switch_main, pump_switch_left, pump_switch_right, flow_meter_left, flow_meter_right, tank_level, oil, gas, homogenity, bearing, microcontroller_id, `utc_timestamp`, created_at, box_id, unit_label, processed,report_date) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?, ?, 0, ?) ON DUPLICATE KEY UPDATE `utc_timestamp` = ?', [$request->latitude, $request->longitude, $speed, $request->altitude, $request->arm_height_left, $request->arm_height_right, $request->temperature_left, $request->temperature_right, $request->pump_switch_main, $request->pump_switch_left, $request->pump_switch_right, $request->flow_meter_left, $request->flow_meter_right, $request->tank_level, $request->oil, $request->gas, $request->homogenity, $request->bearing, $request->microcontroller_id, $request->utc_timestamp, $request->box_id, $request->unit_label, $report_date, $request->utc_timestamp]);
	    	} else {
		      	return response()->json([
		        	'status' 	=> false, 
